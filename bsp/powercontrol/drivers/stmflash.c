...
#endif		 
rt_uint16_t STMFLASH_BUF[STM_SECTOR_SIZE/2];//最多是2K字节
void STMFLASH_Write(rt_uint32_t WriteAddr,rt_uint16_t *pBuffer,rt_uint16_t NumToWrite)	
{
	rt_uint32_t secpos;	   //扇区地址
	rt_uint16_t secoff;	   //扇区内偏移地址(16位字计算)
	rt_uint16_t secremain; //扇区内剩余地址(16位字计算)	   
 	rt_uint16_t i;    
	rt_uint32_t offaddr;   //去掉0X08000000后的地址
