...
char buf_tx[]="asdfasfasdfasdkfla\r\n";
...
extern int TCP_ReConnet(void)
{
		...
		err = sendAtCmdWaitResp("AT+GPSUART=0\r",RT_TICK_PER_SECOND, "OK",10);     //串口1输出
//		if(err != GU620_OK)
//		{
//				GU620_ONOFF = 1;
//				gps_flag = 0;
//				return 0;
//		}
		err = sendAtCmdWaitResp("AT+GPSPWR=0\r",RT_TICK_PER_SECOND,"OK",10);    //GPS断电
//		if(err != GU620_OK)
//		{
//				GU620_ONOFF = 1;
//				gps_flag = 0;
//				return 0;
//		}
		err = sendAtCmdWaitResp("AT+CIPCLOSE=1\r",RT_TICK_PER_SECOND, "OK",30); //首先快速断开TCP链接
		if(err != GU620_OK)
		{
				GU620_ONOFF = 1;
				gps_flag = 0;
		...
		if(p1 != NULL)
		{
			err = sendAtCmdWaitResp(IpPort,RT_TICK_PER_SECOND*2, "CONNECT OK",50);            //重新建立链接TCP	
		if(err != GU620_OK)
		{
				GU620_ONOFF = 1;
				gps_flag = 0;
...
int IP_Process(char ip[20],unsigned short port)
{
	...
	err = sendAtCmdWaitResp("AT+CIPCLOSE=1\r",RT_TICK_PER_SECOND*2, "OK",30); //首先快速断开TCP链接
	if(err != GU620_OK)
	{
			GU620_ONOFF = 1;
			gps_flag = 0;
...
//		return err;	
	...
	if(p1 != NULL)
	{
		err = sendAtCmdWaitResp(IpPort,RT_TICK_PER_SECOND*10, "CONNECT OK",50);            //重新建立链接TCP
		if (err != GU620_OK)
			return err;
...
//		return GU620_ERR;
//	}
//	rt_device_open(GU620_dev,RT_DEVICE_OFLAG_RDWR|RT_DEVICE_FLAG_INT_RX);
	return GU620_OK;
}
...
//	rt_thread_delay(2000);     //等待20s 等待开机
	...
	while(1)
	{
		...
		{
			GU620_on();
//			rt_thread_delay(RT_TICK_PER_SECOND*20);     //等待20s 等待开机
...
//		rt_device_write(GU620_dev,0,"AT+CIPSEND=10\r",rt_strlen(buf_tx));
//		rt_thread_delay(1);
//		rt_device_write(GU620_dev,0,buf_tx,rt_strlen(buf_tx));
