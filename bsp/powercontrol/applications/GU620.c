...
//静态线程的 线程堆栈
static rt_uint8_t GU620[1024];
//静态线程的 线程控制块
static struct rt_thread GU620_thread;
...
//	char ip[]="AT+CIPSTART=\"TCP\",\"111.207.18.18\",56152,2";
	err = sendAtCmdWaitResp("AT+GPSUART=0\r",RT_TICK_PER_SECOND, "OK",5);     //串口1不输出
	if(err != GU620_OK)
		return err;
...
//		return err;	
	...
	err = sendAtCmdWaitResp("ATE0\r",RT_TICK_PER_SECOND, "OK",5);     //关闭回显
	if (err != GU620_OK)
		return err;	
...
int GU620_Thread(void)
{
	...
	rt_pin_write(pGU620,PIN_HIGH);
	rt_err_t result;
	result = rt_thread_init(&GU620_thread,"GU620_thread",rt_thread_entry_GU620,RT_NULL,
						(rt_uint8_t*)&GU620[0],sizeof(GU620),8,10);
//	rt_thread_t rt_GU620;
//	rt_GU620 = rt_thread_create("GU620",&rt_thread_entry_GU620,RT_NULL,
//									1024,8,10);
	if(result == RT_EOK)
		rt_thread_startup(&GU620_thread);
	return GU620_OK;
}
